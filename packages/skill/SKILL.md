...
- **MCP-powered.** 13 MCP tools let AI agents generate, validate, and theme Ignix-Lite UI.
...
### Native HTML Elements (28 components - no custom tag needed)

| Component | HTML Element | `data-intent` Values | Key Attributes |
|-----------|-------------|----------------------|----------------|
| Alert | `<aside>` | `primary` `info` `success` `warning` `danger` | - |
| Accordion | `<details>` | - | `open` |
| Badge | `<mark>` | `success` `warning` `danger` `neutral` | - |
| Badge (status chip) | `<span role="status">` | `success` `warning` `danger` `neutral` | - |
| Breadcrumb | `<nav aria-label="Breadcrumb"> > ol > li > a` | - | `aria-current`, `data-style` |
| Button | `<button>` | `primary` `danger` `warning` `success` `gradient` `neutral` `ghost` | `type`, `disabled`, `aria-busy` |
| Card | `<article>` | - | slot attributes |
| Checkbox | `<input type="checkbox">` inside `<label>` | `success` `danger` `warning` `neutral` | `disabled`, `required`, `checked`, `data-size` (on label) |
...
| Layout | `<div>` / `<section>` / `<aside>` / `<main>` / `<header>` / `<footer>` / `<nav>` / `<form>` / `<label>` / `<article>` / `<figure>` / `<span>` | - | `data-layout`, `data-region`, `data-size`, `data-width`, `data-pad`, `data-gap`, `data-align`, `data-justify`, `data-cols`, `data-min`, `data-wrap`, `data-dense`, `data-side`, `data-height`, `data-stack`, `data-ratio`, `data-pin`, `data-position`, `data-hide`, `data-debug` |
| Meter | `<meter>` | - | `value`, `min`, `max`, `low`, `high`, `optimum` |
| Navigation | `<nav>` | - | `aria-label`, `aria-current` |
| Progress | `<progress>` | - | `value`, `max` |
| Radio | `<input type="radio">` inside `<label>` | `success` `danger` `warning` `neutral` | `disabled`, `required`, `checked`, `data-size` (on label) |
...
| [`assets/intents.json`](assets/intents.json) | `data-intent` values for all 28 components |
| [`references/api-guide.md`](references/api-guide.md) | All 13 MCP tools fully documented |
