...
const tokens = readJSON('packages/skill/assets/design-tokens.json')
if (tokens) {
...
const intents = readJSON('packages/skill/assets/intents.json')
...
const guide = readText('packages/skill/references/api-guide.md')
if (guide) {
...
const examples = readText('packages/skill/references/examples.md')
if (examples) {
  for (const tool of REQUIRED_TOOLS) {
...
console.log('\nCheck 5: SKILL.md ')
const skill = readText('packages/skill/SKILL.md')
if (skill !== null) {
...
const template = readText('packages/skill/templates/component.html.template')
if (template !== null) {
