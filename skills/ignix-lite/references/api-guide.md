...
Ignix-Lite MCP is a **Model Context Protocol** server (`packages/mcp`) that exposes 13 tools for AI assistants to generate, validate and theme Ignix-Lite UI components.
...
```json
{
  "components": [
    ...
    "card", "checkbox", "codeblock", "combobox", "dialog", "divider",
    "dropdown", "form", "grid", "input", "layout", "meter", "navigation",
    "progress", "radio", "select", "skeleton", "tab", "table",
...
Generates Ignix-Lite HTML from a natural language description, runs an auto-correct loop checking it against validation and accessibility rules, and optionally returns a base64 PNG preview.

**Input:**
```json
{
  "description": "a primary button saying Click Me",
  "options": {
    "preview": true,
    "width": 300,
    "theme": "light",
...
```json
{
  ...
  "html": "<button data-intent=\"primary\">Click Me</button>",
  "components_used": ["button"],
  "validation": {
    "valid": true,
    "score": 100,
...
Compares the token size footprint of an Ignix-Lite component snippet with an equivalent Tailwind CSS utility-first layout estimate.
...
```json
{
  "ignix_html": "<button data-intent=\"primary\">Save</button>",
  "ignix_tokens": 6,
  "tailwind_html_estimate": "<button class=\"px-4 py-2 font-medium rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 transition-colors bg-blue-600 hover:bg-blue-700 text-white focus:ring-blue-500\">Save</button>",
  "tailwind_tokens_estimate": 43,
  "tokens_saved": 37,
